...
func WithFileRotationP(file string, maxSize, maxBackup, maxAge int, compress bool, logType string) Option {
	if logType == "daily" {
		name := time.Now().Format("2006-01-02.log")
		file = strings.ReplaceAll(file, "logs.log", name)
	}
	...
	return func(opt *option) {
		opt.file = &lumberjack.Logger{ // concurrent-safed
			...
			MaxBackups: maxBackup,  // 最多保留 300 个备份
